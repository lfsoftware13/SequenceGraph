...
from common.torch_util import SequenceCosineSimilarity
...
class ContextQueryAttention(nn.Module):
    ...
    def __init__(self, hidden_size=128, cosine_sim=False):
...
class CosineCrossAttention(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        self._sim = SequenceCosineSimilarity()

    def forward(self, C, Q):
        S = self._sim(C, Q)
        # S shape [batch, seq_C, seq_Q]

        S_ = F.softmax(S, dim=2)
        # S_ shape [batch, seq_C, seq_Q]
        A = torch.bmm(S_, Q)

        S__ = F.softmax(S, dim=1).permute(0, 2, 1)
        # S__ shape [batch, seq_Q, seq_C]
        B = torch.bmm(S__, C)
