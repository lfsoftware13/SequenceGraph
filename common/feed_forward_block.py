...
class FeedForwardLayer(nn.Module):
    ...
    def forward(self, x):
        tmp = self.linear(x)
        shape = tmp.shape
        tmp = tmp.view(shape[0], -1, shape[-1]).permute(0, 2, 1)
...
class MultiLayerFeedForwardLayer(nn.Module):
    def __init__(self, n_layer, input_dim, hidden_size, output_dim, dropout, activator=nn.ReLU(), last_no_activate=False):
        ...
        self.feed_forward_layers = nn.Sequential(
            *([FeedForwardLayer(input_dim, hidden_size, dropout, activator=activator)] +
              [FeedForwardLayer(hidden_size, hidden_size, dropout, activator=activator) for _ in range(n_layer-2)] +
              [nn.Linear(hidden_size, output_dim)] if last_no_activate else
